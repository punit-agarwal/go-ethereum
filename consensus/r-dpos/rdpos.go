...
// Package rdpos implements the Randomised-proof-of-stake consensus engine.
...
import (
	...
	"github.com/ethereum/ethereum/accounts"
	"github.com/ethereum/ethereum/common"
	"github.com/ethereum/ethereum/common/hexutil"
	"github.com/ethereum/ethereum/consensus"
	"github.com/ethereum/ethereum/consensus/clique"
	"github.com/ethereum/ethereum/consensus/misc"
	"github.com/ethereum/ethereum/core/state"
	"github.com/ethereum/ethereum/core/types"
	"github.com/ethereum/ethereum/crypto"
	"github.com/ethereum/ethereum/crypto/sha3"
	"github.com/ethereum/ethereum/ethdb"
	"github.com/ethereum/ethereum/log"
	"github.com/ethereum/ethereum/params"
	"github.com/ethereum/ethereum/rlp"
	"github.com/ethereum/ethereum/rpc"
)
...
// sigHash returns the hash which is used as input for the Randomised-proof-of-stake
...
// rdpos is the Randomised-proof-of-stake consensus engine proposed to support the
// Ethereum testnet following the Ropsten attacks.
type rdpos struct {
	config *params.rdposConfig // Consensus engine configuration parameters
...
// New creates a rdpos Randomised-proof-of-stake consensus engine with the initial
// signers set to the ones provided by the user.
func New(config *params.rdposConfig, db ethdb.Database) *rdpos {
...
func (c *rdpos) Author(header *types.Header) (common.Address, error) {
	return ecrecover(header, c.signatures)
}

// Get signer coinbase
func (c *rdpos) Signer() common.Address { return c.signer }

// VerifyHeader checks whether a header conforms to the consensus rules.
func (c *rdpos) VerifyHeader(chain consensus.ChainReader, header *types.Header, fullVerify bool) error {
...
func (c *rdpos) VerifyHeaders(chain consensus.ChainReader, headers []*types.Header, fullVerifies []bool) (chan<- struct{}, <-chan error) {
...
func (c *rdpos) verifyHeaderWithCache(chain consensus.ChainReader, header *types.Header, parents []*types.Header, fullVerify bool) error {
...
func (c *rdpos) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header, fullVerify bool) error {
	if common.IsTestnet {
		fullVerify = false
	}
	...
	// Ensure that the block doesn't contain any uncles which are meaningless in rdpos
...
func (c *rdpos) verifyCascadingFields(chain consensus.ChainReader, header *types.Header, parents []*types.Header, fullVerify bool) error {
	...
	*/
	...
	for step := uint64(1); step <= chain.Config().rdpos.Gap; step++ {
...
func (c *rdpos) checkSignersOnCheckpoint(chain consensus.ChainReader, header *types.Header, signers []common.Address) error {
	number := header.Number.Uint64()
...
func (c *rdpos) GetSnapshot(chain consensus.ChainReader, header *types.Header) (*Snapshot, error) {
	number := header.Number.Uint64()
...
func (c *rdpos) StoreSnapshot(snap *Snapshot) error {
...
func (c *rdpos) GetMasternodes(chain consensus.ChainReader, header *types.Header) []common.Address {
...
func (c *rdpos) GetPeriod() uint64 { return c.config.Period }
...
func (c *rdpos) YourTurn(chain consensus.ChainReader, parent *types.Header, signer common.Address) (int, int, int, bool, error) {
	masternodes := c.GetMasternodes(chain, parent)

	if common.IsTestnet {
		// Only three mns hard code for testnet.
...
func (c *rdpos) snapshot(chain consensus.ChainReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *rdpos) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (c *rdpos) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
...
func (c *rdpos) verifySeal(chain consensus.ChainReader, header *types.Header, parents []*types.Header, fullVerify bool) error {
...
func (c *rdpos) GetValidator(creator common.Address, chain consensus.ChainReader, header *types.Header) (common.Address, error) {
...
func (c *rdpos) Prepare(chain consensus.ChainReader, header *types.Header) error {
...
func (c *rdpos) UpdateMasternodes(chain consensus.ChainReader, header *types.Header, ms []Masternode) error {
	number := header.Number.Uint64()
	log.Trace("take snapshot", "number", number, "hash", header.Hash())
	// get snapshot
	snap, err := c.snapshot(chain, number, header.Hash(), nil)
...
func (c *rdpos) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
	// set block reward
	number := header.Number.Uint64()
	rCheckpoint := chain.Config().rdpos.RewardCheckpoint
...
// with.
func (c *rdpos) Authorize(signer common.Address, signFn clique.SignerFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *rdpos) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
	header := block.Header()
...
// current signer.
func (c *rdpos) CalcDifficulty(chain consensus.ChainReader, time uint64, parent *types.Header) *big.Int {
	return c.calcDifficulty(chain, parent, c.signer)
}

func (c *rdpos) calcDifficulty(chain consensus.ChainReader, parent *types.Header, signer common.Address) *big.Int {
...
func (c *rdpos) APIs(chain consensus.ChainReader) []rpc.API {
	return []rpc.API{{
		Namespace: "rdpos",
		Version:   "1.0",
		Service:   &API{chain: chain, rdpos: c},
		Public:    false,
	}}
}

func (c *rdpos) RecoverSigner(header *types.Header) (common.Address, error) {
	return ecrecover(header, c.signatures)
}

func (c *rdpos) RecoverValidator(header *types.Header) (common.Address, error) {
	// If the signature's already cached, return that
	hash := header.Hash()
...
func (c *rdpos) GetMasternodesFromCheckpointHeader(preCheckpointHeader *types.Header, n, e uint64) []common.Address {
...
func (c *rdpos) CacheData(header *types.Header, txs []*types.Transaction, receipts []*types.Receipt) []*types.Transaction {
...
func (c *rdpos) CacheSigner(hash common.Hash, txs []*types.Transaction) []*types.Transaction {
	signTxs := []*types.Transaction{}
	for _, tx := range txs {
		if tx.IsSigningTransaction() {
			signTxs = append(signTxs, tx)
...
func (c *rdpos) GetDb() ethdb.Database {
	return c.db
}
...
func getM1M2(masternodes []common.Address, validators []int64, currentHeader *types.Header, config *params.ChainConfig) (map[common.Address]common.Address, uint64, error) {
	...
	if maxMNs > 0 {
		...
		if isForked {
			moveM2 = ((currentHeader.Number.Uint64() % config.rdpos.Epoch) / uint64(maxMNs)) % uint64(maxMNs)
		}
...
func (c *rdpos) CheckMNTurn(chain consensus.ChainReader, parent *types.Header, signer common.Address) bool {
	masternodes := c.GetMasternodes(chain, parent)
